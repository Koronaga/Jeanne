...
${settingsManager.eventList.map(e => `${e}: ${settings.subbed.includes(e) === true ? 'subscribed' : 'not subscribed'}`).join('\n')}`);
    ...
    if (/\+[^ ]/.test(suffix)) {
      settingsManager.subEvents(suffix.match(/(\+[^ ]+)/g), msg.channel)
        .then(events => {
          bot.createMessage(msg.channel.id, `Subscribed to: \`${events.join('` `')}\``);
        })
        .catch(e => {
    ...
    if (/\-[^ ]/.test(suffix)) {
      settingsManager.unsubEvents(suffix.match(/(-[^ ]+)/g), msg.channel)
        .then(events => {
          bot.createMessage(msg.channel.id, `Unsubscribed from: \`${events.join('` `')}\``);
        })
        .catch(e => {
          bot.createMessage(msg.channel.id, e);
        });
...
module.exports = {
  ...
  usage: "Usage at http://settings.jeannedarc.xyz/",
  aliases: ['set', 'config'],
  cooldown: 3,
